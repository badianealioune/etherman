...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")
        await self.highrise.walk_to(Position(17.0 , 0.5 , 15.5 , "FrontRight"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")   
        await self.highrise.chat(f"Bienvenue {user.username} dans❤️REDLIGHT❤️.Pour danser utilise de 1 à 95.Ecris help pour voir les commandes")
      
        await self.highrise.send_emote("emote-launch")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if        message.startswith("Help") or      message.startswith("/help") or      message.startswith("!help") or message.startswith("help"):
            await self.highrise.chat(f"/list  , /drague,  /emotes, /joke  , /emote-id  , /folie , /mariage?")
        ...
        if message.startswith("kick"):
          if user.username == "ALIOUNE_" "_Chaa_":
              pass
          else:
    ...
    async def on_user_leave(self, user: User) -> None:
        ...
        await self.highrise.chat(f"Au revoir @{user.username} repasse quand tu veux❤️")
        await self.highrise.send_emote("emote-launch")